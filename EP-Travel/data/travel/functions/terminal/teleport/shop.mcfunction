...
execute if score #ep.realistic_enabled ep.config matches 1 run scoreboard players operation #ep.vault ep.variable += #ep.terminal.teleport_cost ep.variable

tag @s add ep.trait.teleported
execute as @e[tag=ep.teleportation_marker] run function travel:terminal/teleport/shop_locator
tag @s remove ep.trait.teleported

function directory:terminal/message/teleport/shop
tag @s[tag=ep.trait.player.teleport.shop] remove ep.trait.player.teleport.shop
