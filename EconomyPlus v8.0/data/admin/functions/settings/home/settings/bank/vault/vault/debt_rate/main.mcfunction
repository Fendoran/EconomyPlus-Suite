...
scoreboard players operation #ep.ui.value ep.variable = #ep.debt.rate ep.variable
execute positioned ^0 ^-0.83 ^ run function admin:settings/home/settings/bank/vault/vault/debt_rate/construct
...
tellraw @p[tag=ep.caster,tag=!ep.ignore.help] [{"text":"\u276cEP\u276d","color":"#D4AF37"}, {"text": " "}, {"text": "This is the interest rate applied when a player is in debt.  This value is applied on an interval.", "color": "#ffffff"}]
