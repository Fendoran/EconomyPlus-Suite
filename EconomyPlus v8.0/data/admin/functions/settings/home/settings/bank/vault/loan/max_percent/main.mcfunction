...
scoreboard players operation #ep.ui.value ep.variable = #ep.max_loan_percent ep.variable
execute positioned ^0 ^-0.83 ^ run function admin:settings/home/settings/bank/vault/loan/max_percent/construct
...
tellraw @p[tag=ep.caster,tag=!ep.ignore.help] [{"text":"\u276cEP\u276d","color":"#D4AF37"}, {"text": " "}, {"text": "This is the percent of the vault the maximum loan can be.  (Whichever value is lower between this and the maximum value is the one that is used.)", "color": "#ffffff"}]
