execute as @e[tag=ep.trait.shop] at @s if score @s ep.session = #ep.target_uid ep.session if score #ep.count ep.variable matches 1..99999999 run function directory:shop/method/set_percent_change
