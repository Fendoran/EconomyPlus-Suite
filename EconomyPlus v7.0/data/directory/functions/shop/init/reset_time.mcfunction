...
execute as @a[tag=ep.player.init] at @s if score @s ep.session = #ep.target_uid ep.owner rotated ~ 0 anchored eyes positioned ^ ^-0.75 ^1.5 as @e[type=area_effect_cloud,tag=ep.shop.link] run function directory:shop/init/reset_time/ui
