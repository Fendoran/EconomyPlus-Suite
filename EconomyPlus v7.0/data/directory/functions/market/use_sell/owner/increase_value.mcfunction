...
execute as @e[type=area_effect_cloud,tag=ep.trait.market,tag=ep.trait.market.target,limit=1] run scoreboard players operation @s ep.stored_money += @s ep.money
...
execute align xyz positioned ~0.5 ~0.5 ~0.5 as @e[type=area_effect_cloud,tag=ep.trait.market,tag=ep.trait.market.target,limit=1] run function directory:shop/draw/current_use
